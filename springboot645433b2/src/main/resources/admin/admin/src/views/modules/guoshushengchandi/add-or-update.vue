	...
	<div class="addEdit-block" :style='{"padding":"30px 0","fontSize":"14px","color":"#999","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="地理位置" prop="diliweizhi">
					<el-input v-model="ruleForm.diliweizhi" placeholder="地理位置" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="水资源" prop="shuiziyuan">
					<el-input v-model="ruleForm.shuiziyuan" placeholder="水资源" clearable  :readonly="ro.shuiziyuan"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="水资源" prop="shuiziyuan">
					<el-input v-model="ruleForm.shuiziyuan" placeholder="水资源" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="气候条件" prop="qihoutiaojian">
					<el-input v-model="ruleForm.qihoutiaojian" placeholder="气候条件" clearable  :readonly="ro.qihoutiaojian"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="气候条件" prop="qihoutiaojian">
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='shuiziyuan'){
							this.ruleForm.shuiziyuan = obj[o];
							this.ro.shuiziyuan = true;
							continue;
						}
						if(o=='qihoutiaojian'){
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `guoshushengchandi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.guoshushengchandiCrossAddOrUpdateFlag = false;
											this.parent.search();
