<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#769589","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('guoshuxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('guoshuxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"4px solid #374f45","width":"100%","padding":"0","margin":"20px 0 0","borderRadius":"8px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="guoshutupian" width="200" label="果蔬图片">
						<template slot-scope="scope">
							<div v-if="scope.row.guoshutupian">
								...
								<img v-else :src="$base.url+scope.row.guoshutupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.guoshutupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('guoshuxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="view" v-if="isAuth('guoshuxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('guoshuxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<shifei-cross-add-or-update v-if="shifeiCrossAddOrUpdateFlag" :parent="this" ref="shifeiCrossaddOrUpdate"></shifei-cross-add-or-update>
		<jiaoshui-cross-add-or-update v-if="jiaoshuiCrossAddOrUpdateFlag" :parent="this" ref="jiaoshuiCrossaddOrUpdate"></jiaoshui-cross-add-or-update>
		<caizhai-cross-add-or-update v-if="caizhaiCrossAddOrUpdateFlag" :parent="this" ref="caizhaiCrossaddOrUpdate"></caizhai-cross-add-or-update>
...
import shifeiCrossAddOrUpdate from "../shifei/add-or-update";
import jiaoshuiCrossAddOrUpdate from "../jiaoshui/add-or-update";
import caizhaiCrossAddOrUpdate from "../caizhai/add-or-update";
	export default {
		data() {
			return {
				...
				shifeiCrossAddOrUpdateFlag: false,
				jiaoshuiCrossAddOrUpdateFlag: false,
				caizhaiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			shifeiCrossAddOrUpdate,
			jiaoshuiCrossAddOrUpdate,
			caizhaiCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			shifeiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shifeiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','guoshuxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shifeiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shifeiCrossaddOrUpdate.init(row.id,type);
				});
			},
			jiaoshuiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jiaoshuiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','guoshuxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiaoshuiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiaoshuiCrossaddOrUpdate.init(row.id,type);
				});
			},
			caizhaiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.caizhaiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','guoshuxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.caizhaiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.caizhaiCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.guoshumingcheng!='' && this.searchForm.guoshumingcheng!=undefined){
            params['guoshumingcheng'] = '%' + this.searchForm.guoshumingcheng + '%'
          }
	...
	this.$router.push({path:'/discussguoshuxinxi',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	guoshuxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'guoshuxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
