...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="灌溉浇水" prop="guangaijiaoshui">
            <el-select v-model="ruleForm.guangaijiaoshui" placeholder="请选择灌溉浇水" :disabled=" false  ||ro.guangaijiaoshui" >
              <el-option
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.guangaishijian = this.getCurDateTime()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='guangaishijian'){
              this.ruleForm.guangaishijian = obj[o];
              this.ro.guangaishijian = true;
              continue;
            }
          }
            this.ruleForm.guangaijiaoshui = 0;
            this.ro.guangaijiaoshui = false;
        }else if(type=='edit'){
			this.info()
		}
        // 获取用户信息
        ...
        this.guangaijiaoshuiOptions = "1".split(',')
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jiaoshui/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
			...
			obj.guangaijiaoshui = parseFloat(obj.guangaijiaoshui) + parseFloat(this.ruleForm.guangaijiaoshui)
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jiaoshui/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(table+`/update`,obj).then(res => {});
								this.$http.post(`jiaoshui/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
