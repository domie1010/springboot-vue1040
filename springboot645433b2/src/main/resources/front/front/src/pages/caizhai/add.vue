...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="采摘数量" prop="zhongzhishuliang">
            <el-input v-model.number="ruleForm.zhongzhishuliang" 
                placeholder="采摘数量" clearable :disabled=" false  ||ro.zhongzhishuliang"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="采摘时间" prop="caizhaishijian">
              <el-date-picker
				  :disabled=" false  ||ro.caizhaishijian"
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.caizhaishijian = this.getCurDateTime()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            this.ruleForm.zhongzhishuliang = 0;
            this.ro.zhongzhishuliang = false;
        }else if(type=='edit'){
			this.info()
		}
        // 获取用户信息
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`caizhai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
			...
			obj.zhongzhishuliang = obj.zhongzhishuliang - this.ruleForm.zhongzhishuliang
			if(obj.zhongzhishuliang<0){
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('caizhai/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								obj.zhongzhishuliang = parseFloat(obj.zhongzhishuliang) - parseFloat(this.ruleForm.zhongzhishuliang)

								this.$http.post(table+`/update`,obj).then(res => {});
								this.$http.post(`caizhai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
