			...
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","color":"#999","textAlign":"left","display":"block","order":"2"}' >
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">生产地点</label>
						<el-input v-model="searchForm.shengchandidian" placeholder="生产地点" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#769589","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('yunshuzhuizong','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('yunshuzhuizong','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"4px solid #374f45","width":"100%","padding":"0","margin":"20px 0 0","borderRadius":"8px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="shengchandidian"
						label="生产地点">
						<template slot-scope="scope">
							{{scope.row.shengchandidian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="xiaoshoudidian"
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="运输时长">
						<template slot-scope="scope">
							{{scope.row.yunshushizhang}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="luxiantu" width="200" label="路线图">
						<template slot-scope="scope">
							<div v-if="scope.row.luxiantu">
								<img v-if="scope.row.luxiantu.substring(0,4)=='http'" :src="scope.row.luxiantu.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.luxiantu.split(',')[0])">
								<img v-else :src="$base.url+scope.row.luxiantu.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.luxiantu.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('yunshuzhuizong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yunshuzhuizong','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.shengchandidian!='' && this.searchForm.shengchandidian!=undefined){
            params['shengchandidian'] = '%' + this.searchForm.shengchandidian + '%'
          }
	...
	yunshuzhuizongstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'yunshuzhuizong/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
